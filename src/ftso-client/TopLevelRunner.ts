...
export class TopLevelRunner {
  ...
  scheduleVotingEpochActions() {
    ...
    setTimeout(async () => {
      this.scheduleVotingEpochActions();
      try {
        await this.onVotingEpoch(nextEpochStartSec + this.epochs.epochDurationSec);
      } catch (e) {
  ...
  async onVotingEpoch(epochDeadlineSec: number) {
    const currentVotingEpochId = this.epochs.priceEpochIdForTime(this.currentTimeSec());
    const currentRewardEpochId = this.epochs.rewardEpochIdForPriceEpochId(currentVotingEpochId);
    this.logger.info(
    ...
    this.logger.info(`[${currentVotingEpochId}] Finished processing voting epoch.`);
  }
  ...
  private async runVotingProcotol(currentEpochId: number, epochDeadlineSec: number) {
    ...
    if (revealData !== undefined) {
      ...
      const result = await this.protocols[0].getResultAfterDeadline(previousEpochId, revealEnd);
