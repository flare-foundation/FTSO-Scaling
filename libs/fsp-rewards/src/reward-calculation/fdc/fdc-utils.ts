...
import { FDCEligibleSigner, FDCOffender, FDCOffense, FDCRewardData } from "../../data-calculation-interfaces";
...
export function extractFDCRewardData(
  ...
  if (voteCounter.size > 0) {
    ...
    const maxBitVotes = [...voteCounter.entries()]
      .filter(([_, count]) => count === maxCount)
      .map(([bitVote, _]) => bitVote);
    maxBitVotes.sort();
  ...
  for (const signature of signatures) {
    ...
    const eligibleSigner: FDCEligibleSigner = {
      ...
      bitVote,
      dominatesConsensusBitVote:
        consensusBitVote === undefined ? undefined : isConsensusVoteDominated(consensusBitVote, bitVote),
  ...
  for (const submission of bitVoteSubmissions) {
    const submitSignatureAddress = rewardEpoch
      .getSubmitSignatureAddressFromSubmitAddress(submission.submitAddress)
      .toLowerCase();
    ...
    const submissionAddress = rewardEpoch
      .getSubmitAddressFromSubmitSignatureAddress(submitSignatureAddress)
      .toLowerCase();
