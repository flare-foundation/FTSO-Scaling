...
export class IndexerClient {
  ...
  public async getSubmit1CallsInRange(fromVotingEpochId: VotingEpochId, toVotingEpochId?: VotingEpochId, endTimeout?: number): Promise<SubmitResponse> {
    ...
    const transactionsResults = await this.queryTransactions(CONTRACTS.Submission, "submit1", startTime, endTime);
    const submits: SubmissionData[] = transactionsResults.map((tx) => {
      const timestamp = tx.timestamp;
      const votingRoundId = EPOCH_SETTINGS.votingEpochForTimeSec(timestamp);
  ...
  public async getReveals(fromVotingEpochId: VotingEpochId, toVotingEpochId?: VotingEpochId, endTimeout?: number): Promise<SubmitResponse> {
