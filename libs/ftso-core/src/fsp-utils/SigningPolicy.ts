...
export namespace SigningPolicy {
  ...
  export function encode(policy: ISigningPolicy) {
    ...
    if (policy.rewardEpochId < 0 || policy.rewardEpochId > 2 ** 24 - 1 || policy.rewardEpochId % 1 !== 0) {
      throw Error(`Reward epoch id out of range: ${policy.rewardEpochId}`);
    }
    if (
      policy.startVotingRoundId < 0 ||
      policy.startVotingRoundId > 2 ** 32 - 1 ||
      policy.startVotingRoundId % 1 !== 0
    ) {
