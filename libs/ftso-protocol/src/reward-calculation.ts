...
export function calculateClaimsForPriceEpoch(
  ...
  const priceEpochOffers = rewardEpochOffers.map((offer) =>
    rewardOfferForPriceEpoch(priceEpochId, offer, epochs)
  );
  ...
  if (finalizerAddress === undefined) {
    ...
    const finalizationClaims = claimsForFinalizer(
      finalizationOffers,
      finalizerAddress,
      priceEpochId
    );
  ...
  const resultClaims = claimsForSymbols(
    priceEpochId,
    calculationResults,
    medianOffers
  );
  ...
  if (committedFailedReveal.length > 0) {
    console.log(
      `Penalizing ${
        committedFailedReveal.length
      } voters for missed reveal: ${Array.from(
...
export function generateProofsForClaims(
  allClaims: readonly RewardClaim[],
  mroot: string,
  claimer: Address
) {
  ...
  function getProof(i: number) {
    ...
    if (!proof)
      throw new Error(`No Merkle proof exists for claim hash ${allHashes[i]}`);
    return proof;
  }
}
 ...
 */
function rewardOfferForPriceEpoch(
  priceEpoch: number,
  offer: RewardOffered,
  epochs: EpochSettings
): RewardOffered {
  ...
  const remainder = offer.amount
    .mod(toBN(epochs.rewardEpochDurationInEpochs))
    .toNumber();
  const firstPriceEpochInRewardEpoch =
    epochs.firstPriceEpochForRewardEpoch(rewardEpoch);
...
function computePenalties(
  ...
  for (const voterWeight of voterWeights.values())
  ...
  for (const voter of committedFailedReveal) {
    ...
    if (voterWeight === undefined)
      throw new Error(`Illegal state: weight for voter ${voter} is undefined.`);

    for (const offer of priceEpochOffers) {
      const penaltyAmount = offer.amount
        .mul(voterWeight)
        .div(totalWeight)
        .mul(MISSED_REVEAL_PENALIZATION);
...
function calculateClaimsForOffer(
  ...
  const elasticBandDiff = toBN(medianPrice)
    .mul(toBN(offer.elasticBandWidthPPM))
    .div(toBN(1000000));
  ...
  if (offer.leadProviders.length > 0) {
    const leadProvidersSet = new Set<string>(
      offer.leadProviders.map((x) => x.toLowerCase())
    );
    ...
    if (pricesOfLeadProvidersThatVoted.length > 0) {
      ...
      const trustedMedianPrice =
        pricesOfLeadProvidersThatVoted[
          Math.floor(pricesOfLeadProvidersThatVoted.length / 2)
        ];
      const eligibleRange = toBN(trustedMedianPrice)
        .mul(toBN(offer.rewardBeltPPM))
        .div(toBN(1000000))
        .toNumber();
  ...
  for (let i = 0; i < calculationResult.voters!.length; i++) {
    ...
    voterRecords.push({
      ...
      iqr:
        ...
        ((price === lowIQR || price === highIQR) &&
          randomSelect(feedId(offer), priceEpoch, voterAddress)),
      pct: price > lowPCT && price < highPCT,
      eligible:
        pricesOfLeadProvidersThatVoted.length === 0
          ? true
          : price >= lowEligible && price <= highEligible,
    } as VoterRewarding);
  }
    ...
    if (pctSum.eq(toBN(0))) {
      ...
      if (voterRecord.iqr) {
        newWeight = newWeight.add(
          offer.iqrSharePPM.mul(voterRecord.weight).mul(pctSum)
        );
      }
      if (voterRecord.pct) {
        newWeight = newWeight.add(
          offer.pctSharePPM.mul(voterRecord.weight).mul(iqrSum)
        );
      }
    }
  ...
  if (!totalReward.eq(offer.amount)) {
    throw new Error(
      `Total reward for ${offer.currencyAddress} is not equal to the offer amount`
    );
  }
...
export function mergeClaims(
  mergePriceEpochId: number,
  unmergedClaims: readonly RewardClaim[]
): RewardClaim[] {
  function mergeClaimsOfSameType(
    claims: RewardClaim[]
  ...
  for (const claim of unmergedClaims) {
    const voterClaimsByCcy =
      claimsByVoterAndCcy.get(claim.beneficiary) ||
      new Map<string, RewardClaim[]>();
  ...
  for (const voterClaimsByCcy of claimsByVoterAndCcy.values()) {
    for (const voterClaims of voterClaimsByCcy.values()) {
      const [penalties, claims] = _.partition(
        voterClaims,
        (claim) => claim instanceof Penalty
      );
      const [fixedClaims, weightedClaims] = _.partition(
        claims,
        (claim) => claim.isFixedClaim === true
      );
      ...
      if (mergedPenalty) {
        ...
        if (remPenalty && mergedWeighted) {
          [mergedWeighted, remPenalty] = applyPenalty(
            mergedWeighted,
            remPenalty
          );
        }
  ...
  const mergedBurnClaim = mergeClaimsOfSameType(
    previousBurnClaims.concat(newBurnClaims)
  );
...
function claimsForSymbols(
  ...
  for (const calculationResult of calculationResults) {
    ...
    for (const offer of offersForSymbol) {
      claims.push(
        ...calculateClaimsForOffer(priceEpoch, offer, calculationResult)
      );
    }
  }
  return claims;
...
function claimRewardsMap(claims: RewardClaim[]) {
  ...
  for (const claim of claims) {
    const amount =
      currencyAddressToTotalReward.get(claim.currencyAddress) || toBN(0);
    currencyAddressToTotalReward.set(
      claim.currencyAddress,
      amount.add(claim.amount)
    );
  }
 ...
 */
function assertOffersVsClaimsStats(
  offers: RewardOffered[],
  claims: RewardClaim[]
) {
  ...
  for (const currencyAddress of offersRewards.keys()) {
    ...
    if (!offerAmount.eq(claimAmount)) {
      throw new Error(
        `offerAmount ${offerAmount} != claimAmount ${claimAmount} for ${currencyAddress}`
      );
    }
  }
}
