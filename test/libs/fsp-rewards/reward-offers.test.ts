...
describe(`Reward offers (${getTestFile(__filename)})`, () => {
  ...
  it("granulatedPartialOfferMap returns a non-empty map", () => {
    assert(granulatedPartialOffers);
  });

  it("should expose 11 feeds per voting round and no entries outside the requested range", () => {
  ...
  it("should expose the correct number of offers per feed", () => {
  ...
  it("should split reward by types", () => {
  ...
  it("should split reward by types for inflation offers", () => {
