...
describe(`generator-rewards (${getTestFile(__filename)})`, () => {
  ...
  before(function () {
    // DO NOT CHANGE THESE SETTINGS — TESTS DEPEND ON THEM.
  ...
  after(() => {
    resetEpochSettings();
    resetEnvVariables();
    // Restore NETWORK so later test suites see the original value. Without this,
    // EPOCH_SETTINGS() keeps recomputing from the (now reset-to-NaN) env vars.
  ...
  it("produces the expected claim distribution for the happy-path scenario", async () => {
    ...
    const indexerClient = new IndexerClientForRewarding(entityManager, requiredHistoryTimeSec, logger);
    const rewardEpochManger = new RewardEpochManager(indexerClient);
    const dataManager = new DataManagerForRewarding(indexerClient, rewardEpochManger, logger);
  ...
  it("penalises the last voter for a reveal offence", async () => {
  ...
  it("penalises the second-to-last voter for double signing", async () => {
  ...
  it("rewards an external finalizer when no voter finalizes the first voting round", async () => {
  ...
  it("produces the same happy-path claims via the persistent-storage code path", async () => {
