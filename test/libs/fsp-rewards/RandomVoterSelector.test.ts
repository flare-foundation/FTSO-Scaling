...
describe(`RandomVoterSelector (${getTestFile(__filename)})`, () => {
  const voters = [
  ...
  it("initialises thresholds, totalWeight, and voter slots from constructor args", () => {
  ...
  it("binary-searches the threshold boundaries for the right voter index", () => {
  ...
  it("generates the expected sequence from a known seed (regression vector)", () => {
    const randomVoterSelector = new RandomVoterSelector(voters, weights, DEFAULT_THRESHOLD_BIPS);
    const protocolId = 1;
  ...
  it.skip("selects voters whose combined weight reaches the threshold (skipped)", () => {
    const randomVoterSelector = new RandomVoterSelector(voters, weights, DEFAULT_THRESHOLD_BIPS);
