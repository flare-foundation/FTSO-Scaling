...
import { testFeeds } from "../../apps/ftso-data-provider/ftso-data-provider.service.test";
import { getTestFile } from "../../utils/getTestFile";

describe(`RewardEpochManager (${getTestFile(__filename)})`, () => {
  ...
  it("loads the correct RewardEpoch for a voting round derived from synthetic events", async () => {
    const rewardEpochId = 1;
    await setUpRewardEpoch(rewardEpochId);

    const epochManager = new RewardEpochManager(new IndexerClient(em, indexerHistorySec, emptyLogger));
