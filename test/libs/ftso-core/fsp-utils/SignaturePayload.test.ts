...
describe(`SignaturePayload (${getTestFile(__filename)})`, () => {
  ...
  it("roundtrips a signature payload through encode/decode", () => {
    const message = {
      protocolId: 15,
      votingRoundId: 1234,
      isSecureRandom: true,
  ...
  it("encodes a payload with empty unsignedMessage", () => {
    const message = {
      protocolId: 15,
      votingRoundId: 1234,
      isSecureRandom: true,
  ...
  it("rejects encode when type or unsignedMessage format is invalid", () => {
    const message = {
      protocolId: 15,
      votingRoundId: 1234,
      isSecureRandom: true,
  ...
  it("rejects decode when input is too short", async () => {
    expect(() => SignaturePayload.decode("0x1234567890")).to.throw("Invalid format - too short");
  });

  it("decodes a sequence of signature payloads from call data", () => {
  ...
  it("augments a payload with signer index, signer address, and message hash", () => {
    const protocolId = 15;
    const votingRoundId = 1234;
  ...
  it("inserts augmented payloads into a list sorted by signer index", () => {
    const protocolId = 15;
    const votingRoundId = 1234;
    const messages: ISignaturePayload[] = [];
    const signerMap = new Map<string, number>();
