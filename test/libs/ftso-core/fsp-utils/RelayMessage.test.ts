...
describe(`RelayMessage (${getTestFile(__filename)})`, () => {
  ...
  it("roundtrips a relay message through encode/decode, with and without newSigningPolicy", () => {
  ...
  it("compares relay messages by signing policy, signatures, and merkle root or new policy", () => {
    const merkleRoot = ethers.hexlify(ethers.randomBytes(32));
    const messageData = {
      protocolId: 15,
      votingRoundId,
  ...
  it("rejects encode with verify=true when signature weight is below threshold", () => {
    const merkleRoot = ethers.hexlify(ethers.randomBytes(32));
    const messageData = {
      protocolId: 15,
      votingRoundId,
